...
const createMocks = (
  ...
  const host = createAppHost(
    [wrappedPackage, ...moreEntryPoints],
    withThrowOnError()
  );
  ...
  return {
    host,
    shell: getShell(),
    renderInShellContext: (reactElement: ReactElement<any>) =>
...
describe("connectWithShell", () => {
  ...
  it("should have shell context outside of main view with renderOutsideProvider option", () => {
    ...
    const PureComp = ({ shellName }: { shellName: string }) => (
      <div className={"my-wrapper"}>{shellName}</div>
    );
    const mapStateToProps = (s: Shell) => ({ shellName: s.name });
  ...
  it("should optimize props comparison", () => {
    ...
    expect(
      testKit.root
        .findByType(ConnectedComp)
        .find((x) => typeof x.children[0] === "string").children[0]
    ).toBe('{"a":1}');
  ...
  it("should pass ownProps to shouldComponentUpdate", () => {
    ...
    const ConnectedComp = connectWithShell<{}, CompOwnProps>(
      undefined,
      undefined,
      shell,
      {
  ...
  describe("arePropsEqualFuncWrapper", () => {
    ...
    beforeEach(() => {
      ...
      innerCompShouldComponentUpdateSpy = jest.fn(
        (ownProps: InnerCompOwnProps) => {}
      );
      ...
      const PureInnerComp: FunctionComponent<InnerCompProps> = ({
        num,
        onClick,
      }) => {
      ...
      const ConnectedInnerComp = connectWithShell(
        undefined,
        mapDispatchToProps,
        shell,
        {
      ...
      const ConnectedOuterComp = connectWithShell(
        mapStateToProps,
        undefined,
        shell,
        {
  ...
  it("should pass scoped state to mapStateToProps", () => {
    ...
    expect(collectAllTexts(parentWrapper)).toContain(
  ...
  it("should re-provide shell context for children of bound component", async () => {
    ...
    const ConnectedUnboundCompWithChildren = connectWithShell<
      ...
      PureCompWithChildrenStateProps
    >(mapStateToProps, undefined, shell, { allowOutOfEntryPoint: true })(
    ...
    expect(
      testKit.root.findByProps({ className: "A" }).props["data-value"]
    ).toBe(getValueFromState(getMockShellState(host)));
    expect(
      testKit.root.findByProps({ className: "B" }).props["data-value"]
    ).toBe(boundShellState.mockValue);
    expect(collectAllTexts(testKit.root)).toContain(
      getValueFromState(getMockShellState(host))
    );
  });
  ...
  it("should render contributed boundary aspect", () => {
    ...
    const { host, shell } = createMocks({
      ...
      attach: (myShell) => {
        myShell.contributeBoundaryAspect((props) => (
          <div className="TEST-ASPECT">{props.children}</div>
        ));
      },
    });
    const PureComp: FunctionComponent<{}> = () => (
      <div className="TEST-PURE-COMP">TEST</div>
    );
    ...
    // assert
    expect(
      testKit.root.findAllByProps({ className: "TEST-ASPECT" }).length
    ).toBe(1);
    expect(
      testKit.root.findAllByProps({ className: "TEST-PURE-COMP" }).length
    ).toBe(1);
  });
  ...
  it("should render multiple contributed boundary aspects", () => {
    ...
    const { host, shell } = createMocks({
      ...
      attach: (myShell) => {
        myShell.contributeBoundaryAspect((props) => (
          <div className="TEST-ASPECT-A">{props.children}</div>
        ));
        myShell.contributeBoundaryAspect((props) => (
          <div className="TEST-ASPECT-B">{props.children}</div>
        ));
      },
    });
    const PureComp: FunctionComponent<{}> = () => (
    ...
    // assert
    expect(
      testKit.root.findAllByProps({ className: "TEST-ASPECT-A" }).length
    ).toBe(1);
    expect(
      testKit.root.findAllByProps({ className: "TEST-ASPECT-B" }).length
    ).toBe(1);
    expect(
      testKit.root.findAllByProps({ className: "TEST-PURE-COMP" }).length
    ).toBe(1);
      ...
      attach: (myShell) => {
        myShell.contributeBoundaryAspect((props) => (
          <div className="TEST-ASPECT">
            <TestAspectContext.Provider value={{ theNumber: 123 }}>
              {props.children}
            </TestAspectContext.Provider>
          </div>
        ));
      },
    });
    const PureComp: FunctionComponent<{}> = () => (
      <TestAspectContext.Consumer>
        {(aspect) => <div className="TEST-PURE-COMP">{aspect.theNumber}</div>}
      </TestAspectContext.Consumer>
    );
    const ConnectedComp = connectWithShell(undefined, undefined, shell, {
      allowOutOfEntryPoint: true,
...
describe("connectWithShell-useCases", () => {
    ...
    attach(shell) {
      shell.contributeState<FirstState>(() => ({
        firstState: (state = { valueOne: "init1" }, action) => {
          return action.type === "SET_FIRST_STATE"
    ...
    attach(shell) {
      shell.contributeState<SecondState>(() => ({
        secondState: (state = { valueTwo: "init2" }, action) => {
          return action.type === "SET_SECOND_STATE"
  ...
  const entryPointFirstObservable: EntryPoint = {
    ...
    attach(shell) {
      const observableThree = shell.contributeObservableState<
        FirstObservableState,
        FirstObservableSelectors
      >(
        () => ({
          firstObservable: (state = { valueThree: "init3" }, action) => {
            return action.type === "SET_FIRST_OBSERVABLE"
              ? { valueThree: action.value }
              : state;
          },
        }),
        (state) => {
  ...
  const entryPointSecondObservable: EntryPoint = {
    ...
    attach(shell) {
      const observableFour = shell.contributeObservableState<
        SecondObservableState,
        SecondObservableSelectors
      >(
        () => ({
          secondObservable: (state = { valueFour: "init4" }, action) => {
            return action.type === "SET_SECOND_OBSERVABLE"
              ? { valueFour: action.value }
              : state;
          },
        }),
        (state) => {
  ...
  const withDependencyAPIs = (
    ...
    return {
      ...ep,
      getDependencyAPIs: () =>
        ep.getDependencyAPIs ? [...ep.getDependencyAPIs(), ...deps] : deps,
    };
  };
  ...
  const PureComp: FunctionComponent<CompProps> = ({
    valueOne,
    valueTwo,
    valueThree,
  }) => {
  ...
  const getComponentValueByClassName = (
    className: string,
    testKit: ReactTestRenderer
  ...
  it("should invoke subscribed callback when observed state changes", () => {
    ...
    shell
      .getAPI(FirstObservableAPI)
      .observables.three.subscribe(shell, (next) => {
  ...
  it("should not notify subscribers when deferring notifications", async () => {
    const { host, shell, renderInShellContext } = createMocks(
      entryPointWithState,
  ...
  it("should not have pending subscribers when starting to defer notifications", async () => {
    ...
    await act(async () => {
      ...
      await host.getStore().deferSubscriberNotifications(() => {
        hasPendingSubscribersWhileDeferring = shell
          .getStore()
  ...
  it("should support nested defered notification actions", async () => {
    ...
    await act(async () => {
      await host.getStore().deferSubscriberNotifications(async () => {
        dispatchAndFlush(
          { type: "SET_FIRST_STATE", value: "update from outer" },
          host
        );
        await host.getStore().deferSubscriberNotifications(() => {
          dispatchAndFlush(
            { type: "SET_FIRST_STATE", value: "update from inner" },
            host
          );
          valueOneInInnerDeferNotifications = getComponentValueByClassName(
            "ONE",
            testKit
          );
        });
        valueOneInOuterDeferNotifications = getComponentValueByClassName(
          "ONE",
          testKit
        );
      });
    ...
    expect(getComponentValueByClassName("ONE", testKit)).toBe(
      "update from inner"
    );
    expect(mapStateToPropsSpy).toHaveBeenCalledTimes(2);
    expect(renderSpy).toHaveBeenCalledTimes(2);
  ...
  it("should clear state memoization on dispatch when deferring notifications and setting shouldDispatchClearCache", async () => {
    const { host, shell } = createMocks(entryPointWithState, [
      entryPointSecondStateWithAPI,
    ]);
    let numberOfCalls = 0;
    const originalFn = jest.fn(() => ++numberOfCalls);
    const memoizedFn = shell.memoizeForState(
      originalFn,
      () => "*"
    ) as _.MemoizedFunction;
  ...
  it("should not update uninterested component on change in observable state", () => {
    const { host, shell, renderInShellContext } = createMocks(
      entryPointWithState,
      [entryPointSecondStateWithAPI, entryPointFirstObservable]
    );
    const ConnectedComp = connectWithShell(mapStateToProps, undefined, shell, {
      allowOutOfEntryPoint: true,
    })(PureComp);
  ...
  it("should update component through observer", () => {
    const { host, shell, renderInShellContext } = createMocks(
      withDependencyAPIs(entryPointWithState, [FirstObservableAPI]),
      [entryPointSecondStateWithAPI, entryPointFirstObservable]
    );
  ...
  it("should throw if observable is read in store subscription", () => {
    const { host, shell, renderInShellContext } = createMocks(
      withDependencyAPIs(entryPointSecondStateWithAPI, [FirstObservableAPI]),
  ...
  it("should update only the relevant observing components", () => {
    const { host, shell, renderInShellContext } = createMocks(
      withDependencyAPIs(entryPointFirstObservable, [SecondObservableAPI]),
...
describe("observeWithShellPureComponent", () => {
  ...
  const observableEntryPoint: EntryPoint = {
    ...
    attach(shell) {
      const observableState = shell.contributeObservableState<
        ActualObservableState,
        ObservableAPISelectors
      >(
        () => ({
  ...
  it("should update observing component", () => {
    const { host, shell, renderInShellContext } =
      createMocks(observableEntryPoint);
    ...
    expect(
      testKit.root.findByProps({ id: "OBSERVED_NUMBER" }).children[0]
    ).toBe("1");
    expect(
      testKit.root.findByProps({ id: "OBSERVED_STRING" }).children[0]
    ).toBe("init");
  ...
  it("should add props using mapShellToStaticProps", () => {
    ...
    type ComponentProps = ObservedSelectorsMap<ObservableAPI> &
    ...
    const PureComponent: FunctionComponent<ComponentProps> = (props) => {
      return (
        <div>
          <button
            onClick={() => props.onClick(props.observable.getNumberValue())}
          />
        </div>
      );
    };

    const { host, shell, renderInShellContext } = createMocks(myAPIEntryPoint, [
      observableEntryPoint,
    ]);
    ...
    >(
      ...
      (
        funcShell,
        ownProps?: ObservedSelectorsMap<ObservableAPI>
      ): ComponentShellStaticProps => {
