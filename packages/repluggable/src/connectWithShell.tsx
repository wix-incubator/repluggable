...
type Returns<T> = () => T;
export type MapStateToProps<State, OwnProps, StateProps> = Maybe<
  (shell: Shell, state: State, ownProps?: OwnProps) => StateProps
>;
...
>;
type MapShellToStaticProps<ShellStaticProps, OwnProps> = Maybe<
  (shell: Shell, ownProps?: OwnProps) => ShellStaticProps
>;
...
function wrapWithShouldUpdate<
  Props extends unknown,
  F extends (next: Props, prev: Props) => void
>(
  ...
  return ((...args: Parameters<F>) =>
    shouldUpdate && !shouldUpdate(shell, getOwnProps())
      ? true
      : func(args[0], args[1])) as F;
}

function arePropsEqualFuncWrapper<
  Props extends unknown,
  F extends (next: Props, prev: Props) => boolean
>(
  ...
  return ((...args: Parameters<F>) => {
    const componentShouldUpdate = componentShouldUpdateFunc(
      shell,
      getOwnProps()
    );
...
function wrapWithShellContext<State, OwnProps, StateProps, DispatchProps>(
    ...
    implements WrapperMembers<State, OwnProps, StateProps, DispatchProps>
  {
    ...
    constructor(props: WrappedComponentOwnProps<OwnProps>) {
      ...
      this.mapStateToProps = mapStateToProps
        ? (__, ownProps?) => {
            return this.props.shell.log.monitor(
              `connectWithShell.mapStateToProps ${this.props.shell.name}`,
              {},
              () =>
                mapStateToProps(
                  this.props.shell,
                  this.props.shell.getStore<State>().getState(),
                  ownProps
                )
            );
          }
      ...
      this.mapDispatchToProps = mapDispatchToProps
        ? (dispatch, ownProps?) => {
            return this.props.shell.log.monitor(
              `connectWithShell.mapDispatchToProps ${this.props.shell.name}`,
              {},
      ...
      const shouldComponentUpdate =
        ...
        this.props.shell.memoizeForState(
          options.shouldComponentUpdate,
          () => "*"
        );
      ...
      const memoWithShouldUpdate = <F extends AnyFunction>(f: F): F => {
        ...
        return ((...args) => {
          if (
            ...
            !shouldComponentUpdate(this.props.shell, this.getOwnProps())
          ) {
            return last;
          }
          last = f(...args);
      ...
      this.connectedComponent = reduxConnect<
        StateProps,
        DispatchProps,
        OwnProps,
        State
    ...
    private readonly getOwnProps = () =>
      _.omit(
        this.props,
        "shell"
      ) as unknown as React.PropsWithChildren<OwnProps> &
  ...
  const wrapChildrenIfNeeded = (
    props: WithChildren<OwnProps>,
    originalShell: Shell
  ): WithChildren<OwnProps> =>
    props.children
      ? {
          ...props,
          children: (
            <ShellContext.Provider value={originalShell}>
              {props.children}
      ...
      {(shell) => {
        return (
          ...
          >
            {
              <ConnectedComponent
                {...wrapChildrenIfNeeded(props, shell)}
                shell={boundShell}
              />
            }
          </ErrorBoundary>
...
function wrapWithShellRenderer<OwnProps>(
  ...
  boundShell as PrivateShell;
  return (props: WithChildren<OwnProps>) => (
    <ShellRenderer
      host={(boundShell as PrivateShell).getAppHost()}
      shell={boundShell}
...
type ComponentWithChildrenProps<OwnProps> = (
  props: WithChildren<OwnProps>
) => JSX.Element;

export type ConnectedComponentFactory<
  State = {},
  OwnProps = {},
  StateProps = {},
  DispatchProps = {},
  OwnPropsPure = OwnProps
> = (
 ...
 */
export function connectWithShell<
  State = {},
  OwnProps = {},
  StateProps = {},
  DispatchProps = {}
>(
  ...
  const validateLifecycle = (component: React.ComponentType<any>) => {
    if (
      boundShell.wasInitializationCompleted() &&
      !options.allowOutOfEntryPoint
    ) {
      const componentText =
  ...
  return (
    ...
    const wrappedWithShellContext = wrapWithShellContext(
      component,
      mapStateToProps,
      mapDispatchToProps,
      boundShell,
...
function mapObservablesToSelectors<M extends ObservablesMap>(
  map: M,
  allowUnsafeReading?: boolean
): ObservedSelectorsMap<M> {
...
>(
  ...
  innerFactory?: ConnectedComponentFactory<
    State,
    OwnProps,
    StateProps,
    DispatchProps
  ...
  State,
  OmitObservedSelectors<OwnProps, MappedObservables>,
  StateProps,
  DispatchProps,
  OwnProps
> {
  type ObservableWrapperProps = OmitObservedSelectors<
    OwnProps,
    MappedObservables
  >;
  ...
  const observableConnectedComponentFactory: ConnectedComponentFactory<
    ...
    OwnProps
  > = (pureComponent) => {
    const ConnectedComponent: React.ComponentType<any> = innerFactory
      ? innerFactory(pureComponent)
      : pureComponent;

    class ObservableWrapperComponent extends React.Component<
      ObservableWrapperProps,
    ...
    > {
      ...
      constructor(props: OwnProps) {
        ...
        this.unsubscribes = [];
        this.state = {
          ...mapObservablesToSelectors(observables, true),
          rendersCounter: 0,
        };
        this.staticShellProps = mapShellToStaticProps
          ? mapShellToStaticProps(boundShell, props)
          : ({} as ShellStaticProps);
      }
      ...
      public componentDidMount() {
        for (const key in observables) {
          const unsubscribe = observables[key].subscribe(boundShell, () => {
            ...
            this.setState((state) => ({
              ...newState,
              rendersCounter: ((state.rendersCounter as number) + 1) % 1000,
            }));
          });
    ...
    const hoc = (
      props: WithChildren<OmitObservedSelectors<OwnProps, MappedObservables>>
    ) => {
      return (
...
>(
  ...
  OwnProps
> {
  return createObservableConnectedComponentFactory(
    observables,
    boundShell,
    mapShellToStaticProps
  );
}
...
>(
  ...
  OwnProps
> {
  const innerFactory = connectWithShell(
    mapStateToProps,
    mapDispatchToProps,
    boundShell,
    options
  );
  const wrapperFactory = observeConnectedComponentWithShell<
    MappedObservables,
    OwnProps
  >(
    observables,
