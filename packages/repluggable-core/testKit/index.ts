...
export const createAppHost: typeof _createAppHost = (
  packages,
  options = emptyLoggerOptions
) => {
...
function forEachDeclaredAPI(
  allPackages: EntryPointOrPackage[],
  iteration: (dependency: AnySlotKey, entryPoint: EntryPoint) => void
) {
  _.forEach(_.flatten(allPackages), (entryPoint: EntryPoint) => {
    _.forEach(
      entryPoint.declareAPIs ? entryPoint.declareAPIs() : [],
      (dependency) => {
...
export const getPackagesDependencies = (
  ...
  while (entryPointsQueue.length) {
    ...
    const dependencies = currEntryPoint.getDependencyAPIs
      ? currEntryPoint.getDependencyAPIs()
      : [];
    const dependencyEntryPoints = dependencies.map((API: AnySlotKey) =>
      apiToEntryPoint.get(API.name)
    );
...
export function createAppHostWithPacts(
...
export async function createAppHostAndWaitForLoading(
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      const unreadyAPIs = declaredAPIs.filter(
        (api) => !readyAPIs.some((readyAPI) => readyAPI.name === api.name)
      );
      const rootUnreadyAPI =
        globalThis.repluggableAppDebug.utils.getRootUnreadyAPI();

      reject(
        new Error(
          `createAppHostAndWaitForLoading - waiting for loading timed out.
there's a high chance this missing API is the main reason for it: ${JSON.stringify(
            rootUnreadyAPI
          )}

in addition here's the full list of declared APIs that have not been contributed: ${JSON.stringify(
            unreadyAPIs
          )}`
        )
      );
...
export const addMockShell = (
  host: AppHost,
  entryPointOverrides: EntryPointOverrides = {}
): Shell => {
  let shell = null;
  host.addShells([
    {
  ...
  if (!shell) {
    const dependencies = entryPointOverrides.getDependencyAPIs
      ? entryPointOverrides.getDependencyAPIs()
      : [];
