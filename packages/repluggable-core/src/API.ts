...
>;
export type ReactComponentContributor<TProps = {}> = (
  props?: TProps
) => React.ReactNode;
export type ReducersMapObjectContributor<
  TState = {},
  TAction extends Redux.AnyAction = Redux.AnyAction
> = () => Redux.ReducersMapObject<TState, TAction>;
...
export interface ObservableState<TSelectorAPI> {
  subscribe(
    fromShell: Shell,
    callback: StateObserver<TSelectorAPI>
...
export interface Shell
  extends Pick<
    AppHost,
    Exclude<keyof AppHost, "getStore" | "log" | "options">
  > {
  /**
  ...
  contributeAPI<TAPI>(
    key: SlotKey<TAPI>,
    factory: () => TAPI,
    options?: ContributeAPIOptions<TAPI>
  ): TAPI;
  /**
   ...
   */
  lazyEvaluator<F extends AnyFunction, T extends ReturnType<F>>(
    func: F
  ): Lazy<T>;
}
...
export interface EntryPointInterceptor {
  ...
  interceptGetDependencyAPIs?(
    innerGetDependencyAPIs?: EntryPoint["getDependencyAPIs"]
  ): EntryPoint["getDependencyAPIs"];
  interceptDeclareAPIs?(
    innerDeclareAPIs?: EntryPoint["declareAPIs"]
  ): EntryPoint["declareAPIs"];
