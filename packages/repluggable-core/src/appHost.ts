...
export const makeLazyEntryPoint = (
  name: string,
  factory: LazyEntryPointFactory
): LazyEntryPointDescriptor => {
  return {
    name,
    factory,
...
export function createAppHost(
  ...
  options: AppHostOptions = { monitoring: {} }
): AppHost {
  ...
  const entryPointsInstallationEndCallbacks: Map<string, () => void> =
    new Map();
  ...
  const createExtensionSlotsMap = (): Map<AnySlotKey, AnyExtensionSlot> => {
    ...
    const proxyHandler = {
      get(
        target: Map<AnySlotKey, AnyExtensionSlot>,
        propertyName: string | symbol
      ) {
        if (propertyName === "set") {
          return function (key: AnySlotKey, value: AnyExtensionSlot) {
            return batchDeclarationsChangedCallbacks(() =>
        ...
        const originalProperty =
          target[propertyName as keyof Map<AnySlotKey, AnyExtensionSlot>];
  ...
  const declarationsChangedCallbacks = new Map<
    string,
    DeclarationsChangedCallback
  >();
  const APILayers = new WeakMap<AnySlotKey, APILayer[] | undefined>();

  const memoizedFunctions: IterableWeakMap<AnyFunction, MemoizedFunctionData> =
    new IterableWeakMap();
  let shouldFlushMemoization = false;

  const hostAPI: AppHostAPI = {
    getAllEntryPoints: () =>
      [...addedShells.entries()].map(([, { entryPoint }]) => entryPoint),
    getAppHostOptions: () => options,
  };
  const appHostServicesEntryPoint = createAppHostServicesEntryPoint(
    () => hostAPI
  );
  ...
  const memoize = <T extends AnyFunction>(
    ...
    Partial<_.MemoizedFunction> &
    Partial<MemoizeMissHit> => {
  ...
  function lazyEvaluator<F extends AnyFunction, T extends ReturnType<F>>(
    fn: F
  ): Lazy<T> {
    let _value: T;
  ...
  function isLazyEntryPointDescriptor(
    value: AnyEntryPoint
  ): value is LazyEntryPointDescriptor {
    return typeof (value as LazyEntryPointDescriptor).factory === "function";
  }

  function enrichMemoization<
    T extends _.MemoizedFunction & Partial<MemoizeMissHit>
  >(memoized: T): T & MemoizeMissHit {
...
`
    ...
    const setter = memoizedWithMissHit.cache.set.bind(
      memoizedWithMissHit.cache
    );
  ...
  function addShells(
    ...
    !options.disableCheckCircularDependencies &&
      !options.experimentalCyclicMode &&
      validateCircularDependency(allEntryPoints);

    const [lazyEntryPointsList, readyEntryPointsList] = _.partition(
      entryPoints,
      isLazyEntryPointDescriptor
    ) as [LazyEntryPointDescriptor[], EntryPoint[]];

    executeInstallShell(readyEntryPointsList);
    lazyEntryPointsList.forEach(registerLazyEntryPoint);

    setInstalledShellNames(
      getInstalledShellNames().concat(_.map(lazyEntryPointsList, "name"))
    );
  ...
  function isAllAPIDependenciesAreReadyOrPending(
    ...
    const declarers = pendingEntryPoints.flatMap((ep) =>
      (ep.declareAPIs?.() || []).map((k) => [k, ep] as const)
    );
    const [, keyDeclarerEntryPoint] =
      declarers.find(([k, ep]) => _.isEqual(k, checkedKey)) || [];
    if (!keyDeclarerEntryPoint) {
      return false;
    }

    const dependencies =
      keyDeclarerEntryPoint.getDependencyAPIs &&
      keyDeclarerEntryPoint.getDependencyAPIs();
    const uncheckDependencies = _.differenceWith(
      dependencies,
      passed,
      _.isEqual
    );
    ...
    const everyDependenciesReadyOrPending = _.every(
      ...
      (k) =>
        ...
        isAllAPIDependenciesAreReadyOrPending(
          k,
          pendingEntryPoints,
          passed.concat(checkedKey)
        )
    );
      ...
      (entryPoint) => {
        ...
        return _.every(
          ...
          (k) =>
            readyAPIs.has(getOwnSlotKey(k)) ||
            (options.experimentalCyclicMode &&
              isAllAPIDependenciesAreReadyOrPending(k, entryPoints))
        );
      }
    );
    ...
    batchDeclarationsChangedCallbacks(() => {
      ...
      try {
        invokeEntryPointPhase(
          ...
          shells,
          (f) =>
            f.entryPoint.getDependencyAPIs &&
  ...
  function executeShellsChangedCallbacks() {
    shellsChangedCallbacks.forEach((f) =>
      f(
        _.keys(
          InstalledShellsSelectors.getInstalledShellsSet(getStore().getState())
        )
      )
    );
  }
  ...
  function onDeclarationsChanged(
    callback: DeclarationsChangedCallback
  ): UnsubscribeFromDeclarationsChanged {
    ...
    const newSlot = registerSlotOrThrow(key, () =>
      createExtensionSlot(key, host, {
        ...
        customItemsDataStructure:
          options.plugins?.extensionSlot?.customItemsDataStructure,
      })
    );
    return newSlot;
  }
  ...
  function declareCustomSlot<TItem>(
    ...
    const newSlot = registerSlotOrThrow(key, () =>
      createCustomExtensionSlot(key, handler, host, declaringShell)
    );
    return newSlot;
  }
  ...
  function registerSlotOrThrow<TItem, TSlot extends AnyExtensionSlot>(
    key: SlotKey<TItem>,
    factory: () => TSlot
  ): TSlot {
  ...
  function hasShell(name: string): boolean {
    const installedShellsSet = InstalledShellsSelectors.getInstalledShellsSet(
      getStore().getState()
    );
  ...
  function validateCircularDependency(entryPoints: AnyEntryPoint[]): void {
    ...
    entryPoints.forEach((ep) => {
      ...
      declaredApis.forEach((d) =>
        dependencies.forEach((y) => graph.addConnection(d, y))
      );
    });
    ...
    for (const scc of sccs) {
      if (scc.length > 1) {
        ...
        if (circle) {
          ...
          host.log.log(
            "error",
            `Circular API dependency found: ${reversedCircle.join(" -> ")}`
          );
  ...
  function buildStore(): Store {
    ...
    if (store) {
      ...
      store.syncSubscribe(() => {
        ...
        if (
          isStoreSubscribersNotifyInProgress ||
          isObserversNotifyInProgress ||
          shouldFlushMemoizationSync
        ) {
          shouldFlushMemoization = false;
          flushMemoizedForState();
        }
  ...
  function clearCache(
    memoizedFunction: Partial<_.MemoizedFunction> & Partial<MemoizeMissHit>
  ) {
  ...
  function getAPIContributor<TAPI>(key: SlotKey<TAPI>): Shell | undefined {
    const ownKey = getOwnSlotKey(key);
    return extensionSlots.has(ownKey)
      ? _.get(getSlot<TAPI>(ownKey).getSingleItem(), "shell")
      : undefined;
  }

  function doesExtensionItemBelongToShells(
  ...
  function discardSlotKey<T>(key: SlotKey<T>) {
    ...
    host.log.log(
      "verbose",
      `-- Removed slot keys: ${slotKeyToName(ownKey)} --`
    );
  }
  ...
  function isShellBeingDependantOnInGroup(
    declaringShell: PrivateShell,
    shells: PrivateShell[]
  ): boolean {
  ...
  function executeDetachOnShellReadyForRemoval(
  ...
  function executeUninstallShells(names: string[]): void {
    batchDeclarationsChangedCallbacks(() => {
      ...
      const shellsCandidatesToBeDetached = _(names)
        ...
        .compact()
        .flatMap<PrivateShell>((shell) => [
          shell,
          ...findDependantShells(shell),
        ])
        .uniqBy("name")
        .value();
  ...
  function removeShells(names: string[]): Promise<void> {
    ...
    setUninstalledShellNames(
      _.difference(shellNames, getInstalledShellNames())
    );
    ...
    const shell: PrivateShell = {
      ...
      getSlot<TItem>(key: SlotKey<TItem>): ExtensionSlot<TItem> {
        ...
        if (!declaringShell || declaringShell !== shell) {
          throw new Error(
            `Shell '${shell.name}' is trying to get slot '${slotKeyToName(
              key
            )}' that is owned by '${
      ...
      verifyPendingEntryPointsAPIsMismatch:
        host.verifyPendingEntryPointsAPIsMismatch,
      ...
      hasAPI<TAPI>(key: SlotKey<TAPI>): boolean {
        return (
          (dependencyAPIs.has(key) || isOwnContributedAPI(key)) &&
          host.hasAPI(key)
        );
      },
      ...
      contributeAPI<TAPI>(
        ...
        const areSameLayers = (
          ...
          _.isEqual(
            _(l1).castArray().sort().value(),
            _(l2).castArray().sort().value()
          );
        ...
        if (!isInstallingEntryPoints) {
          ...
          setInstalledShellNames(
            _.difference(
              shellNames,
              _.map(unReadyEntryPointsStore.get(), "name")
            )
          );
        }
      ...
      >(
        ...
        const observableUniqueName = `${
          entryPoint.name
        }/observable_${nextObservableId++}`;
        const observable = createObservable(
          shell,
          observableUniqueName,
          mapStateToSelectors
        );
        observable.subscribe(shell, () => {
          flushMemoizedForState();
        });
        const protectedObservable: PrivateObservableState<
          TState,
          TSelectorAPI
        > = {
      ...
      getStore<TState>(): ScopedStore<TState> {
        return {
          dispatch: (
            host.getStore() as PrivateThrottledStore
          ).dispatchWithShell(shell),
          ...
          deferSubscriberNotifications:
            host.getStore().deferSubscriberNotifications,
        };
      },

      contributeMainView(
        fromShell: Shell,
        contributor: ReactComponentContributor
      ): void {
      ...
      memoizeForState(func, resolver, shouldClear?) {
        ...
        memoizedFunctions.set(
          memoized,
          shouldClear ? { f: memoized, shouldClear } : { f: memoized }
        );
        return memoized;
      },
      flushMemoizedForState,
  ...
  function normalizeApiName(name: string) {
    return (
      ...
      name.substring(1).replace(new RegExp(" ", "g"), "")
    );
  }
  ...
  function verifyEntryPointAPIsMismatch(entryPoint: EntryPoint): void {
    ...
    const mismatchedAPI = dependencies.find((dependencyAPI) => {
      ...
      const foundDeclaredAPIKeyWithSameName = slotKeysByName.get(
        slotKeyToName(dependencyAPI)
      );
      const isDeclaredAPIReady = foundDeclaredAPIKeyWithSameName
        ? readyAPIs.has(foundDeclaredAPIKeyWithSameName)
        : false;

      return (
        isPrivateAPI &&
        isDeclaredAPIReady &&
        dependencyAPI !== foundDeclaredAPIKeyWithSameName
      );
    });
