...
import {
  AppHostAPI,
  AppHostServicesEntryPointName,
  AppHostServicesProvider,
} from "../src/appHostServices";
import { ConsoleHostLogger } from "../src/loggers";
import {
  createCircularEntryPoints,
  createDirectCircularEntryPoints,
} from "./appHost.mock";
...
const createHostWithDependantPackages = (DependencyAPI: AnySlotKey) => {
  ...
  return {
    host: createAppHost(
      [dependentPackage, deeplyDependentPackage, helperEntryPoint],
      testHostOptions
    ),
    dependentPackage,
...
describe("App Host", () => {
  ...
  describe("Packages Installation", () => {
    it("should NOT throw on circular dependency if check is disabled in host options", () => {
      ...
      expect(() =>
        createAppHost(circularPackages, hostOptionsWithDisabledCircularCheck)
      ).not.toThrow();
    });
    ...
    it("should throw on direct circular API dependency (private keys)", () => {
      const circularPackages = createDirectCircularEntryPoints();
      expect(() =>
        createAppHost(circularPackages, testHostOptions)
      ).toThrowError();
    });
    ...
    it("should not install multiple shells with the same name", () => {
      expect(() =>
        createAppHost(
          [mockPackage, _.pick(mockPackage, "name")],
          testHostOptions
        )
      ).toThrow();
    });
    ...
    it("should install lazy shells", () => {
      const lazyEntryPoint = makeLazyEntryPoint(
        mockPackage.name,
        async () => mockPackage
      );
  ...
  describe("EntryPoint lifecycle state", () => {
    const takeEntryPointStateSnapshot = (
      shell: Shell
    ): EntryPointStateSnapshot => {
      return {
    ...
    ({ testCase, dependencyAPI, providerPackage }) => {
      describe(`Dependency entry point installation (${testCase})`, () => {
        it("should not install dependent entry point until dependency is installed", async () => {
          const { host, dependentPackage } =
            createHostWithDependantPackages(dependencyAPI);
        ...
        it("should install all dependent entry points chain when dependencies are installed from entry point", async () => {
          const { host, dependentPackage, deeplyDependentPackage } =
            createHostWithDependantPackages(dependencyAPI);
  ...
  describe("Host extension slots", () => {
    ...
    it("should retrieve all slot keys", () => {
      const sortSlotKeys = (slotKeys: AnySlotKey[]) =>
        _.sortBy(slotKeys, "name");

      const host = createAppHost([mockPackage], testHostOptions);

      const actual = sortSlotKeys(host.getAllSlotKeys());
      const expected = sortSlotKeys([
        AppHostAPI,
        mainViewSlotKey,
        stateSlotKey,
        MockAPI,
    ...
    describe("private API slot key", () => {
      ...
      it("should not equal another key with same name that claims it is public", () => {
        ...
        expect(() => host.getAPI(fakeKey1)).toThrowError(
          new RegExp(MockAPI.name)
        );
        expect(() => host.getAPI(fakeKey2)).toThrowError(
          new RegExp(MockAPI.name)
        );
        expect(() => host.getAPI(fakeKey3)).toThrowError(
          new RegExp(MockAPI.name)
        );
      });
    });
    ...
    describe("public API slot key", () => {
      ...
      it("should not equal another key with same name than does not claim it is public", () => {
        ...
        expect(() => host.getAPI(anotherKey1)).toThrowError(
          new RegExp(MockPublicAPI.name)
        );
        expect(() => host.getAPI(anotherKey2)).toThrowError(
          new RegExp(MockPublicAPI.name)
        );
        expect(() => host.getAPI(anotherKey3)).toThrowError(
          new RegExp(MockPublicAPI.name)
        );
      });
    });
  ...
  describe("Entry Point Contributions", () => {
    ...
    it("should execute detach and attach sequence according to dependencies", async () => {
      ...
      const host = createAppHost(
        [dependantEntryPoint2, mockPackage, dependantEntryPoint],
        testHostOptions
      );
    ...
    it("should contribute state", async () => {
      const getMockShellState = (host: AppHost) =>
        _.get(
          host.getStore().getState(),
          [mockPackage.name, mockShellStateKey],
          null
        );
    ...
    describe("memoizeForState", () => {
      ...
      function contributeMemoizedAPI(
        host: AppHost,
        shouldClear?: () => boolean
      ): Shell {
      ...
      const createMockShell = (host: AppHost) => {
        let observableState: ObservableState<ObservableValueSelector> =
          {} as any;
        const mockShell = addMockShell(host, {
          declareAPIs: () => [memoizedAPI],
          attach(shell) {
            observableState = shell.contributeObservableState<
    ...
    describe("memory cleanup:", () => {
      ...
      it("should remove memoized function from memory when there is no ref to memoized function", () => {
        ...
        const createAPI = (shell: Shell): NewAPI => {
          memFn1 = shell.memoizeForState(
            () => ({}),
            _.stubTrue,
            _.stubTrue
          ) as _.MemoizedFunction;
          memFn2 = shell.memoizeForState(
            () => ({}),
            _.stubTrue,
            _.stubTrue
          ) as _.MemoizedFunction;
  ...
  describe("Entry Point Shell Scoping", () => {
    ...
    it("should get scoped state", (done) => {
      ...
      const entryPointWithState: EntryPoint = {
        ...
        extend(shell: Shell) {
          expect(_.get(shell.getStore().getState(), MOCK_STATE_KEY)).toBe(
            state
          );
          done();
        },
    ...
    it("should not be able to uninstall not own installed packages", () => {
      ...
      expect(() =>
        createAppHost(
          [mockPackage, packageThatTriesToUninstallAPackage],
          testHostOptions
        )
      ).toThrow();
    });
      ...
      const shell1 = addMockShell(host, {
        attach(shell) {
          shell.contributeState<State1>(() => ({
            state1: (
              state: ScopedState = INITIAL_STATE,
              action
            ): ScopedState => {
              switch (action.type) {
                case SET_VALUE:
  ...
  describe("API layer", () => {
    ...
    it("should not allow adding shell of unknown layer", () => {
      ...
      expect(() => host.addShells([EntryPoint1])).toThrowError(
        `Cannot find layer ${EntryPoint1.layer}`
      );
    });
    ...
    it("should enforce cross-multi-dimensional-layers name uniqueness", () => {
      ...
      expect(() =>
        ...
        `Cannot initialize host with non unique layers: NOT_UNIQUE`
      );
    });
  ...
  describe("API version", () => {
    it("should provide API of matching version", async () => {
      ...
      expect(() => {
        ...
        new RegExp(
          `Extension slot with key '${SecondMockAPIv2.name}\\\(v${SecondMockAPIv2.version}\\\)' already exists`
        )
      );
    });
  });
  ...
  describe("Host API", () => {
    ...
    it("should get all entry points", async () => {
      const host = createAppHost([mockPackage], testHostOptions) as AppHost &
        AppHostServicesProvider;
      ...
      expect(_.sortBy(allEntryPoints, "name")).toEqual(
        _.sortBy(
          [
            mockPackage,
            mockPackageWithPublicAPI,
            (host.getAppHostServicesShell() as PrivateShell).entryPoint,
          ],
          "name"
        )
      );
    ...
    it("should get host options", () => {
      ...
      expect(host.getAPI(AppHostAPI).getAppHostOptions()).toEqual(
        testHostOptions
      );
    });
  });
...
If the API is intended to be public, it should be declared as "public: true" in the API key, and built in both bundles.`
  ...
  describe("Custom Items Data Structure via AppHost Options (plugins)", () => {
    it("should use customItemsDataStructure from appHostOptions when contributing and removing items", async () => {
      ...
      const host = createAppHost(
        [slotOwnerEntryPoint, ContributorEntryPoint, ListenerEntryPoint],
        {
          monitoring: {},
          plugins: {
            extensionSlot: {
