...
describe('Installed Shells State', () => {
  it('should toggle shells according to shell toggles', () => {
    const actualState = installedShellsReducer(
      initialState,
      InstalledShellsActions.updateInstalledShells(shellsToggleSet),
    );
  ...
  it('should select installed shells from state', () => {
    ...
    expect(InstalledShellsSelectors.getInstalledShellsSet(rootState)).toEqual(
      initialState.installedShells,
    );
  });
});
