...
function createAppHostImpl(): AppHost {
  ...
  async function ensureLazyShellsInstalled(names: string[]) {
    const lazyLoadPromises = names
      .filter(name => !addedShells.has(name))
      .map(loadLazyShell);
  ...
  function getPerShellReducersMapObject(): ShellsReducersMap {
    ...
    return stateSlot
      .getItems()
      .reduce((reducersMap: ShellsReducersMap, item) => {
  ...
  function getCombinedShellReducers(): ReducersMapObject {
    ...
    return Object.keys(shellsReducerMaps).reduce(
      (reducersMap: ReducersMapObject, shellName: string) => {
  ...
  function executeUninstallShells(names: string[]): void {
    ...
    invokeEntryPointPhase(
      'detach',
      names.map(name => addedShells.get(name)) as PrivateShell[],
      f => _.invoke(f.entryPoint, 'detach', f),
    );

    const APIsToDiscard = [...readyAPIs].filter(APIKey =>
      _.includes(names, _.get(getAPIContributor(APIKey), 'name')),
    );
    extensionSlots.forEach(extensionSlot =>
      (extensionSlot as ExtensionSlot<any>).discardBy(extensionItem =>
        doesExtensionItemBelongToShells(extensionItem, names),
      ),
    );
  ...
  function createShell(entryPoint: EntryPoint): PrivateShell {
    ...
    const isOwnContributedAPI = <TAPI>(key: SlotKey<TAPI>): boolean =>
    ...
    const shell: PrivateShell = {
      ...
      removeShells(names: string[]): void {
        ...
        if (!_.isEmpty(namesNotInstalledByCurrentEntryPoint)) {
          throw new Error(
            `Shell ${
              entryPoint.name
            } is trying to uninstall shells: ${names} which is are not installed by entry point ${
              entryPoint.name
            } - This is not allowed`,
          );
        }
      ...
      contributeAPI<TAPI>(key: SlotKey<TAPI>, factory: () => TAPI): TAPI {
        ...
        if (!_.includes(_.invoke(entryPoint, 'declareAPIs') || [], key)) {
          throw new Error(
            `Entry point '${entryPoint.name}' is trying to contribute API '${
              key.name
        ...
        if (canInstallReadyEntryPoints) {
          ...
          executeInstallShell(unReadyEntryPoints);
          setInstalledShellNames(
            _.difference(shellNames, _.map(unReadyEntryPoints, 'name')),
          );
        }
      ...
      contributeState<TState>(
        contributor: ReducersMapObjectContributor<TState>,
      ): void {
