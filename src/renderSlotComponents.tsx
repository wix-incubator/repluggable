...
export const ShellRenderer: React.FunctionComponent<ShellRendererProps> = ({
  shell,
  component,
  key,
  name,
...
export function renderSlotComponents(
  slot: ExtensionSlot<ReactComponentContributor>,
): React.ReactNode[] {
  return slot
    .getItems()
    .map((item, index) => (
...
const SlotRendererPure: React.FunctionComponent<SlotRendererPureProps<any>> = ({
  items,
  mapFunc,
  filterFunc,
}) => (
...
export const SlotRenderer = connect(
...
const mapPredicateHocStateToProps = (
  state: any,
  ownProps: ConnectedPredicateHocProps<any>,
): PredicateHocProps => ({
...
const ConnectedPredicateHoc = connect(mapPredicateHocStateToProps)(
  PredicateHoc,
);
